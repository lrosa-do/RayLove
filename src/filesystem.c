...
static void* dir_read(mount_t *mnt, const char *filename, int *size)
{
  ...
  if (!fp)
  {
    ...
    SDL_LogWarn(0, "FILEIO: [%s] Failed to read file", filename);
    return NULL;
  }
  /* Get size */
...
static int tar_stream_read(mtar_t *tar, void *data, unsigned size) {
  ...
  unsigned res = SDL_RWread(tm->fp,data, 1, size);
...
static int tar_stream_seek(mtar_t *tar, unsigned offset) {
  tar_mount_t *tm = tar->stream;
  int res = SDL_RWseek(tm->fp, tm->offset + offset, RW_SEEK_SET);
...
static int tar_mount(mount_t *mnt, const char *path) {
  ...
  if (err) {
    int offset;
    char buf[4] = "";
    SDL_RWseek(fp, -8, RW_SEEK_END);
    SDL_RWread(fp,buf, 1, 4);
    SDL_RWread(fp,&offset, 1, 4);
    if ( !memcmp(buf, "TAR\0", 4) ) {
      SDL_RWseek(fp, -offset, RW_SEEK_END);
      tm->offset = SDL_RWtell(fp);
    }
    mtar_rewind(tar);
  ...
  while ( (mtar_read_header(tar, &h)) == MTAR_ESUCCESS ) {
    ...
    if (n >= cap) {
      cap = cap ? (cap << 1) : 16;
      tm->map = realloc(tm->map, cap * sizeof(*tm->map));
    }
    /* Store entry */
    strip_trailing_slash(h.name);
