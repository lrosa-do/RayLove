...
void *_dmt_realloc(void *ptr, size_t sz, const char *file, unsigned line) {
  ...
  if (!_dmt_has_node(node)) {
   SDL_LogWarn(0, "Bad realloc: %p %s, line %u\n", ptr, file, line);
    _dmt_abort();
  }
#endif
  ...
  if (node == NULL) {
#ifdef DMT_ABORT_NULL
   SDL_LogWarn(0, "Couldn't reallocate: %s, line %u\n", file, line);
    _dmt_abort();
#else
    return NULL;
#endif
...
void _dmt_free(void *ptr, const char *file, unsigned line) {
  ...
  if (!_dmt_has_node(node))
  {
    SDL_LogWarn(0, "Bad free: %p %s, line %u\n", ptr, file, line);
    _dmt_abort();
  }
#endif
...
void dmt_dump()
{
  ...
  while (node != NULL) {
    SDL_LogWarn(0,"Unfreed: %p %s, line %lu (%lu bytes)\n",
  ...
  SDL_Log("Total unfreed: %lu bytes\n", (unsigned long)total);
}
...
#ifndef DMT_UNSAFE
  if (!_dmt_has_node(node)) {
    SDL_LogWarn(0, "Bad pointer: %p %s, line %u\n", ptr, file, line);
    _dmt_abort();
  }
#endif
