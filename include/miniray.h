...
RLAPI void ProcessInputEvents(const SDL_Event* e);
...
RLAPI long GetFileSize(const char *fileName);                  // Get file modification time (last write time)
...
RLAPI  int DrawBezier( const double *x,const double *y, int n, int s,Color c);
...
RLAPI void rlsetBlendsMode(int mode, int alphamode);
...
RLAPI void rlLoadExtensions();            // Load OpenGL extensions (loader function required)
